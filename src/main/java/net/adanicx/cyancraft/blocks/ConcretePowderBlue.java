...
public class ConcretePowderBlue extends GenericGravityBlock implements IConfigurable {
	
	
	
	public ConcretePowderBlue(){
		super(CMaterials.concretepowder);
		setHardness(0.5F);
		setResistance(2.5F);
		setStepSound(soundTypeSand);
		setBlockTextureName("concrete_powder_blue");
		setBlockName(Utilities.getUnlocalizedName("concretepowderblue"));
        ...
        if (World.getBlock(x, y, z) == this)
        {
			...
			if (this.blockMaterial == CMaterials.concretepowder)
            {
                ...
                if (flag)
                {
                    ...
                    if (l == 0)
                    {
                        World.setBlock(x, y, z, CBlocks.concreteblue);
                    }
                }
            }
        }
