...
public class ConcretePowderPink extends GenericGravityBlock implements IConfigurable {
	
	
	
	public ConcretePowderPink(){
		super(CMaterials.concretepowder);
		setHardness(0.5F);
		setResistance(2.5F);
		setStepSound(soundTypeSand);
		setBlockTextureName("concrete_powder_pink");
		setBlockName(Utilities.getUnlocalizedName("concretepowderpink"));
        ...
        if (World.getBlock(x, y, z) == this)
        {
			...
			if (this.blockMaterial == CMaterials.concretepowder)
            {
                ...
                if (flag)
                {
                    ...
                    if (l == 0)
                    {
                        World.setBlock(x, y, z, CBlocks.concretepink);
                    }
                }
            }
        }
