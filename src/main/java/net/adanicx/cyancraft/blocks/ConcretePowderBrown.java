...
public class ConcretePowderBrown extends GenericGravityBlock implements IConfigurable {
	
	
	
	public ConcretePowderBrown(){
		super(CMaterials.concretepowder);
		setHardness(0.5F);
		setResistance(2.5F);
		setStepSound(soundTypeSand);
		setBlockTextureName("concrete_powder_brown");
		setBlockName(Utilities.getUnlocalizedName("concretepowderbrown"));
        ...
        if (World.getBlock(x, y, z) == this)
        {
			...
			if (this.blockMaterial == CMaterials.concretepowder)
            {
                ...
                if (flag)
                {
                    ...
                    if (l == 0)
                    {
                        World.setBlock(x, y, z, CBlocks.concretebrown);
                    }
                }
            }
        }
