...
public class ConcretePowderSilver extends GenericGravityBlock implements IConfigurable {
	
	
	
	public ConcretePowderSilver(){
		super(CMaterials.concretepowder);
		setHardness(0.5F);
		setResistance(2.5F);
		setStepSound(soundTypeSand);
		setBlockTextureName("concrete_powder_silver");
		setBlockName(Utilities.getUnlocalizedName("concretepowdersilver"));
        ...
        if (World.getBlock(x, y, z) == this)
        {
			...
			if (this.blockMaterial == CMaterials.concretepowder)
            {
                ...
                if (flag)
                {
                    ...
                    if (l == 0)
                    {
                        World.setBlock(x, y, z, CBlocks.concretesilver);
                    }
                }
            }
        }
