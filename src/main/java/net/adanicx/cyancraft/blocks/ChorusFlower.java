...
public class ChorusFlower extends Block implements IConfigurable {
	...
	public ChorusFlower() {
		...
		setBlockName(Utilities.getUnlocalizedName("chorus_flower"));
	...
	public void updateTick(World world, int x, int y, int z, Random rand) {
		...
		if (!canBlockStay(world, x, y, z))
			...
			if (lowerBlock == Blocks.end_stone)
				canGrowUp = true;
			else if (lowerBlock == CBlocks.chorus_plant) {
				int par8 = 1;
				int height;
				for (height = 0; height < 4; height++) {
					Block b = world.getBlock(x, y - (par8 + 1), z);
					if (b != CBlocks.chorus_plant) {
			...
			if (canGrowUp && isSpaceAroundFree(world, x, y + 1, z, ForgeDirection.DOWN) && world.isAirBlock(x, y + 2, z)) {
				world.setBlock(x, y, z, CBlocks.chorus_plant);
				...
				if (grew)
					world.setBlock(x, y, z, CBlocks.chorus_plant, 0, 3);
				else
