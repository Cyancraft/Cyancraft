...
public class GrassPath extends Block implements IConfigurable {
	...
	public GrassPath() {
		...
		setBlockName(Utilities.getUnlocalizedName("grass_path"));
		setBlockBounds(0.0F, 0.0F, 0.0F, 1.0F, 0.9375F, 1.0F);
		setCreativeTab(CyancraftMod.enableGrassPath ? CyancraftMod.creativeTab : null);
	}
	...
	public boolean isEnabled() {
		return CyancraftMod.enableGrassPath;
	}

	public static void onPlayerInteract(PlayerInteractEvent event) {
		if (CyancraftMod.enableGrassPath)
				...
				if (event.action == Action.RIGHT_CLICK_BLOCK)
					if (world.getBlock(event.x, event.y, event.z) == Blocks.grass) {
						...
						if (stack != null && (stack.getItem() instanceof ItemSpade || isTinkersShovel(stack))) {
							world.setBlock(event.x, event.y, event.z, CBlocks.grass_path);
	...
	private static boolean isTinkersShovel(ItemStack stack) {
		if (CyancraftMod.isTinkersConstructLoaded) {
