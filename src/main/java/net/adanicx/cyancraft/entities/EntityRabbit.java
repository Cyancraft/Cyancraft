package net.adanicx.cyancraft.entities;
...
import net.adanicx.cyancraft.entities.AI.BlockPos;
import net.adanicx.cyancraft.entities.AI.EntityAIMoveToBlock;
...
public class EntityRabbit extends EntityAnimal {
	...
	protected String getJumpingSound() {
		return ModReferences.MOD_ID + ":mob.rabbit.hop";
	}

	@Override
	protected String getLivingSound() {
		return ModReferences.MOD_ID + ":mob.rabbit.idle";
	}

	@Override
	protected String getHurtSound() {
		return ModReferences.MOD_ID + ":mob.rabbit.hurt";
	}

	@Override
	protected String getDeathSound() {
		return ModReferences.MOD_ID + ":mob.rabbit.death";
	}
	...
	protected void dropFewItems(boolean hitRencently, int fortune) {
		...
		for (int i = 0; i < j; i++)
			dropItem(CItems.rabbit_hide, 1);

		j = rand.nextInt(2);

		for (int i = 0; i < j; i++)
			if (isBurning())
				dropItem(CItems.cooked_rabbit, 1);
			else
				dropItem(CItems.raw_rabbit, 1);

		if (rand.nextInt(100) <= 10 + fortune)
			entityDropItem(new ItemStack(CItems.rabbit_foot), 0.0F);
	}
