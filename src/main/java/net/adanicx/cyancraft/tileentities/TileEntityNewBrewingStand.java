	...
	private boolean canBrew() {
		if (fuel > 0 && inventory[3] != null && inventory[3].stackSize > 0) {
			...
			if (!itemstack.getItem().isPotionIngredient(itemstack))
				return false;
			else if (itemstack.getItem() == CItems.dragon_breath) {
				for (int i = 0; i < 3; i++)
	...
	private void brewPotions() {
		...
		if (canBrew()) {
			for (int i = 0; i < 3; i++)
				if (inventory[i] != null && inventory[i].getItem() instanceof ItemPotion) {
					int j = inventory[i].getItemDamage();
					if (ItemPotion.isSplash(j) && inventory[3].getItem() == CItems.dragon_breath)
						inventory[i] = new ItemStack(CItems.lingering_potion, inventory[i].stackSize, inventory[i].getItemDamage());
					else {
			...
			worldObj.playSound(xCoord, yCoord, zCoord, ModReferences.MOD_ID + ":block.brewing_stand.brew", 1.0F, 1.0F, true);
		}
	}
