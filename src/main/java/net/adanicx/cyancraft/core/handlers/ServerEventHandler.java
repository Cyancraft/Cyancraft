...
import net.adanicx.cyancraft.CEnchantments;
...
import net.adanicx.cyancraft.entities.AI.EntityAIOpenCustomDoor;
...
public class ServerEventHandler {
	...
	public void onPlayerPickXP(PlayerPickupXpEvent event) {
		CEnchantments.onPlayerPickupXP(event);
	}

	@SubscribeEvent
	public void livingUpdate(LivingUpdateEvent event) {
		CEnchantments.onLivingUpdate(event.entityLiving);
	...
	public void arrowNock(ArrowNockEvent event) {
		...
		for (int i = 0; i < invt.getSizeInventory(); i++) {
			...
			if (stack.getItem() == Items.arrow)
				return;
			if (stack.getItem() == CItems.tipped_arrow) {
	...
	public void arrowLoose(ArrowLooseEvent event) {
		...
		for (int i = 0; i < invt.getSizeInventory(); i++) {
			...
			if (arrow != null && arrow.stackSize > 0 && arrow.getItem() == CItems.tipped_arrow) {
	...
	public void onPlayerLoadFromFileEvent(PlayerEvent.LoadFromFile event) {
		if (!CyancraftMod.enableEnchants)
			return;
		try {
			File file = event.getPlayerFile(ModReferences.MOD_ID);
			if (!file.exists()) {
				file.createNewFile();
				return;
			}
	...
	public void harvestEvent(BlockEvent.HarvestDropsEvent event) {
		if (CyancraftMod.enableSilkTouchingMushrooms && event.isSilkTouching)
			if (event.block == Blocks.brown_mushroom_block) {
				event.drops.clear();
				event.drops.add(new ItemStack(CBlocks.brown_mushroom_block));
			} else if (event.block == Blocks.red_mushroom_block) {
				event.drops.clear();
				event.drops.add(new ItemStack(CBlocks.red_mushroom_block));
			}

		if (CyancraftMod.enableSticksFromDeadBushes)
		...
		if (CyancraftMod.enableShearableCobwebs)
	...
	public void dropEvent(LivingDropsEvent event) {
		...
		if (CyancraftMod.enableSkullDrop)
			dropHead(event.entityLiving, event.source, event.lootingLevel, event.drops);

		Random rand = event.entityLiving.worldObj.rand;
		if (CyancraftMod.enableMutton && event.entityLiving instanceof EntitySheep) {
			int amount = rand.nextInt(3) + 1 + rand.nextInt(1 + event.lootingLevel);
			for (int i = 0; i < amount; i++)
				if (event.entityLiving.isBurning())
					addDrop(new ItemStack(CItems.cooked_mutton), event.entityLiving, event.drops);
				else
					addDrop(new ItemStack(CItems.raw_mutton), event.entityLiving, event.drops);
		}
	}
	...
	public void spawnEvent(EntityJoinWorldEvent event) {
		if (event.entity instanceof EntityPig) {
			...
			if (CyancraftMod.enableBeetroot)
				pig.tasks.addTask(4, new EntityAITempt(pig, 1.2, CItems.beetroot, false));
		} else if (event.entity instanceof EntityChicken) {
			EntityChicken chicken = (EntityChicken) event.entity;
			if (CyancraftMod.enableBeetroot)
				chicken.tasks.addTask(3, new EntityAITempt(chicken, 1.0D, CItems.beetroot_seeds, false));
	...
	public void interactEntityEvent(EntityInteractEvent event) {
		...
		if (!animal.isChild()) {
			if (animal instanceof EntityPig) {
				if (stack.getItem() == CItems.beetroot && CyancraftMod.enableBeetroot)
					setAnimalInLove(animal, event.entityPlayer, stack);
			} else if (animal instanceof EntityChicken)
				if (stack.getItem() == CItems.beetroot_seeds && CyancraftMod.enableBeetroot)
					setAnimalInLove(animal, event.entityPlayer, stack);
		} else if (CyancraftMod.enableBabyGrowthBoost && isFoodItem(animal, stack))
	...
	private boolean isFoodItem(EntityAnimal animal, ItemStack food) {
		if (animal.isBreedingItem(food))
			return true;
		else if (animal instanceof EntityPig && food.getItem() == CItems.beetroot && CyancraftMod.enableBeetroot)
			return true;
		else if (animal instanceof EntityChicken && food.getItem() == CItems.beetroot_seeds && CyancraftMod.enableBeetroot)
			return true;
		else
			return false;
	}
	...
	public void entityHurtEvent(LivingHurtEvent event) {
		if (!CyancraftMod.enableDmgIndicator)
			return;
		...
		if (event.source instanceof EntityDamageSource) {
			...
			if (attacker instanceof EntityPlayer && !(attacker instanceof FakePlayer)) {
				...
				for (int i = 0; i < amount; i++) {
					...
					CyancraftMod.networkWrapper.sendToAllAround(new DamageParticlesMessage(x, y, z), new TargetPoint(player.worldObj.provider.dimensionId, x, y, z, 64));
				}
			}
		}
	}
	...
	public void entityStruckByLightning(EntityStruckByLightningEvent event) {
		if (CyancraftMod.enableVillagerTurnsIntoWitch && event.entity instanceof EntityVillager) {
			EntityVillager villager = (EntityVillager) event.entity;
